  ...
  try {
    ...
    if (!serviceKey) {
      ...
      if (!resp.ok) {
        ...
        const allRows = await resp.json() as Array<{ id: string; name: string | null; company: string | null; domain: string | null; is_archived: boolean | null }>;
        const rows = allRows
          .filter(row => {
            const label = `${row.company ?? ''} ${row.name ?? ''}`.trim();
            return !/^SMOKE-TEST/i.test(label);
          })
          .slice(0, 5);
        if (!rows || rows.length === 0) {
          ...
          for (const row of rows) {
            const name = (row.company || row.name || '').toLowerCase().trim();
...
async function runCortexThetaChecks(page: Page, serviceKey?: string): Promise<CheckResult[]> {
  ...
  // CHECK 6: [CORRECTNESS] Latest theta_sessions row has no parse_error.
  // Mirrors CHECK 5's "latest row" philosophy — only the most recent run matters.
  // Scoping to the single latest row avoids permanent false-FAILs from resolved
  // historical errors. Page-text grep is NOT used — it trips on this assertion's own text.
  try {
...
async function runFleetChecks(page: Page): Promise<CheckResult[]> {
  ...
  // shell renders, so checks run before data appears without this wait.
  // Conditional to this function so other pages see no added latency.
  await Promise.race([
    page.waitForSelector(
      'table tbody tr, [class*="agent"], [class*="Agent"], [class*="card"], [class*="Card"], [role="row"]:not([role="columnheader"]), [role="listitem"]',
      { timeout: 12000 }
    ).catch(() => {}),
    new Promise<void>(r => setTimeout(r, 12000)),
  ]);
