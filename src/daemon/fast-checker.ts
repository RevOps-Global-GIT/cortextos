...
export class FastChecker {
  ...
  // FIX: warn=75, restart=90 to avoid collision; override via env vars
   ...
   *      warnings at CTX_WARN_PCT/CTX_EMERGENCY_PCT, restart at CTX_RESTART_PCT
   *      with delay scaled by severity, post-restart verification + retry.
  ...
  private watchdogCheck(): void {
    ...
    if (ctxPct > 0) {
      if (ctxPct >= this.CTX_WARN_PCT && !this.ctxAlert70Sent && this.telegramApi && this.chatId) {
        ...
        this.log(`WATCHDOG: context at ${ctxPct}% — sending warn heads-up`);
        this.telegramApi
      ...
      if (ctxPct >= this.CTX_EMERGENCY_PCT && !this.ctxAlert85Sent && this.telegramApi && this.chatId) {
        ...
        this.log(`WATCHDOG: context at ${ctxPct}% — sending emergency alert`);
        this.telegramApi
  ...
  private armSafetyNet(reason: string, delayMs: number): void {
    ...
    const alertRecipient = process.env.CTX_ALERT_RECIPIENT || 'operator';
    const instruction = `SYSTEM: Context threshold reached (${reason}). Before restarting: 1) Write handoff files. 2) Notify ${alertRecipient} via Telegram. 3) Run: cortextos bus hard-restart --reason '${reason}'`;
