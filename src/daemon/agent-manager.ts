...
export class AgentManager {
  ...
  private agents: Map<string, { process: AgentProcess; checker: FastChecker; poller?: TelegramPoller; activityPoller?: TelegramPoller; slackPoller?: SlackPoller; pollerToken?: string; activityPollerToken?: string; telegramApi?: TelegramAPI; chatId?: string; telegramRejectCount?: number; telegramLastRejectAlertAt?: number }> = new Map();
  ...
  /**
   * Per-agent in-flight stopAgent() promise. When a startAgent() arrives for
   * the same name while a stop is still running, the new start awaits this
   * promise before proceeding, which eliminates the IPC-level race where
   * `cortextos stop foo && cortextos start foo` arrives at the daemon faster
   * than the PTY can actually shut down (6-15s) and lands both handlers in
   * parallel. PR #11 closed the PTY-level race inside AgentProcess.stop(),
   * but the IPC server (ipc-server.ts) dispatches both handlers
   * fire-and-forget, so the registry-overlap window in AgentManager can
   * still trip without this serialization. See startAgent() / stopAgent()
  ...
  /** Per-agent stale-heartbeat watcher timers (started in startAgent, cleared in stopAgent). */
  private staleWatchers: Map<string, NodeJS.Timeout> = new Map();
  /**
   * Tracks the most recent health-triggered or stale-heartbeat restart timestamp
   * per agent (epoch ms). Enforces a 5-minute cooldown between auto-restarts
   * so a single misbehaving agent cannot hammer restartAgent() in a tight loop.
   */
  private lastRestartAt: Map<string, number> = new Map();
  /**
   * Signal that a restart was initiated by the stale-heartbeat watcher (not a
   * user or crash-recovery restart). Used to skip BOT_TOKEN re-validation when
   * telegramApi was already configured before the restart.
   */
  private healthRestartInProgress: Set<string> = new Set();
  /**
   * OAuth self-heal watchdog (upstream #643). A single daemon-level timer that
   * classifies every registered Claude-runtime agent as healthy/unhealthy via
   * heartbeat age + recent OAuth/login failure signatures in stdout, then
   * refreshes the claude-profile token and restarts only the affected agent.
   * oauthRecoveryInProgress guards against re-entrant recovery on the same
  ...
  async startAgent(name: string, agentDir: string, config?: AgentConfig, org?: string): Promise<void> {
    // BUG-011 race fix: the cortextos IPC server (ipc-server.ts) dispatches
    // stop-agent and start-agent handlers fire-and-forget, so
    //     cortextos stop foo && cortextos start foo
    // can arrive at the daemon with both handlers executing in parallel.
    // stopAgent() is typically still awaiting PTY exit (6-15s) when
    // startAgent() fires. Without serializing here, startAgent would land
    // on `if (this.agents.has(name))` (true, because stopAgent has not yet
    // reached agents.delete) and bail into the pendingRestarts fallback,
    // noisily emitting the BUG-011 REGRESSION CHECK warning for what is in
    // fact the expected concurrent-IPC pattern. The same pattern fires on
    // restart-all and on rapid dashboard toggles.
    //
    // Awaiting the in-flight stop here lets the usual fresh-start path run
    // cleanly. The pendingRestarts fallback below is kept as a safety net
    // for any future code path that mutates this.agents without going
    // through stopAgent (none known today).
    ...
    if (inFlightStop) {
      ...
      try {
        ...
        // proceed to the fresh-start path so the agent is not stuck off.
      }
    }
    ...
    if (this.agents.has(name)) {
      ...
      // longer reaches this branch. The remaining triggers are:
      //   * post-daemon-crash discoverAndStart overlap (expected, quiet log)
      //   * a code path that mutates this.agents without going through
      //     stopAgent (none known today; warn so we notice immediately)
      if (this.daemonJustCrashed) {
        console.log(`[agent-manager] ${name} already in registry (post-crash discovery overlap, expected). Queueing restart.`);
      } else {
        console.warn(`[agent-manager] BUG-011 REGRESSION CHECK: ${name} still in registry during startAgent — pendingRestarts queueing engaged. This should not happen with PR #11 + inFlightStops in place.`);
      }
    ...
    // PM2 process.env fallback: if BOT_TOKEN was not found or failed format
    // validation from the .env file, check whether creds are injected into the
    // process environment (e.g. via PM2 ecosystem env block or a systemd
    // EnvironmentFile). We also skip re-validation entirely when a health-restart
    // is in progress and telegramApi was already configured pre-restart.
    if (!telegramApi) {
      if (this.healthRestartInProgress.has(name)) {
        // Health-triggered restart: telegramApi was validated before the restart;
        // skip the credential block entirely and let the caller re-wire it below.
        log(`[${name}] Health restart in progress — skipping BOT_TOKEN re-validation`);
      } else {
        // Check for per-agent env var (e.g. DEV_BOT_TOKEN for agent named "dev")
        const agentNameUpper = name.toUpperCase().replace(/-/g, '_');
        const envBotToken = process.env[`${agentNameUpper}_BOT_TOKEN`] ?? process.env.BOT_TOKEN;
        const envChatId = process.env[`${agentNameUpper}_CHAT_ID`] ?? process.env.CHAT_ID ?? chatId;
        const envAllowedUser = process.env[`${agentNameUpper}_ALLOWED_USER`] ?? process.env.ALLOWED_USER ?? allowedUserId;

        if (envBotToken && envChatId) {
          log(`[${name}] Using BOT_TOKEN from process env (PM2 fallback)`);
          telegramApi = new TelegramAPI(envBotToken);
          chatId = envChatId;
          if (envAllowedUser) allowedUserId = envAllowedUser;
          log(`Telegram configured via process env (chat_id: ****${String(envChatId).slice(-4)}, allowed_user: ${envAllowedUser ? 'enabled' : 'not set'})`);
        }
      }
    }
   ...
   * name can await it instead of racing into the BUG-011 pendingRestarts
   * fallback. Parallel stopAgent() calls for the same name coalesce onto
   * the same promise (idempotent), so back-to-back stops do not duplicate
   * tear-down work.
   */
  ...
  private async _doStopAgent(name: string): Promise<void> {
    ...
    // With the inFlightStops mechanism in startAgent(), the common IPC
    // stop+start race no longer routes through pendingRestarts (the new
    // start awaits the in-flight stop first). This branch now only fires
    // for the post-crash discoverAndStart overlap case and for any future
    // code path that mutates this.agents without going through stopAgent.
  ...
  // --- OAuth self-heal watchdog (upstream #643) ---

  /**
   * Start the daemon-level OAuth self-heal watchdog. Disabled with
   * CORTEXTOS_OAUTH_SELF_HEAL=0; interval is env-tunable. A single unref'd
   * timer covers every registered Claude-runtime agent.
   */
  ...
  private async runOAuthSelfHealCheck(): Promise<void> {
    ...
    const cooldownMs = parsePositiveInt(process.env.CORTEXTOS_OAUTH_RECOVERY_COOLDOWN_MS, AgentManager.HEALTH_RESTART_COOLDOWN_MS);
    ...
    // while Codex/Hermes/script agents are ignored.
  ...
  private async recoverClaudeOAuthStall(name: string, reason: ClaudeAgentUnhealthyReason): Promise<void> {
    ...
    // Share the manager-wide restart cooldown so the OAuth watchdog and any
    // other health-restart path do not double-restart the same agent.
    ...
    try {
      ...
      const watchdogPaths = resolvePaths('watchdog', this.instanceId, this.resolveAgentOrg(name));
   ...
   */
  async spawnWorker(name: string, dir: string, prompt: string, parent?: string, model?: string, home?: string): Promise<void> {
    ...
    const config: { model?: string; home?: string } = {};
  ...
  /**
   * Start the stale-heartbeat watcher for `agentName`.
   *
   * Every 60 seconds, reads {ctxRoot}/state/{agent}/heartbeat.json and checks
   * whether `last_heartbeat` is older than `health.stale_heartbeat_minutes`
   * (default 45). If the agent is currently `running` AND the heartbeat is
   * stale, triggers a restartAgent() call — subject to a 5-minute cooldown
   * to prevent rapid-fire restarts.
   *
   * After the restart, resets the crash count so the agent is not penalised
   * for pre-restart crashes and clears the healthRestartInProgress flag.
   */
  private startStaleHeartbeatWatcher(agentName: string, config: AgentConfig): void {
    if (this.staleWatchers.has(agentName)) return; // already running
    ...
    const timer = setInterval(async () => {
      const entry = this.agents.get(agentName);
      if (!entry) {
        // Agent was stopped — watcher will be cleared by stopStaleHeartbeatWatcher
        return;
      }
      ...
      try {
        ...
        const hb = JSON.parse(raw) as { last_heartbeat?: string };
        if (!hb.last_heartbeat) return;
        lastHeartbeatMs = new Date(hb.last_heartbeat).getTime();
      } catch {
        return; // unreadable / malformed — skip this cycle
      }

      const ageMs = Date.now() - lastHeartbeatMs;
      if (ageMs <= staleMs) return; // heartbeat is fresh
      ...
      console.log(`[stale-watcher] ${agentName}: heartbeat stale for ${Math.round(ageMs / 60000)} min (threshold: ${staleMinutes} min) — triggering restart`);
      this.lastRestartAt.set(agentName, Date.now());
      this.healthRestartInProgress.add(agentName);

      try {
        await this.restartAgent(agentName);
        // Reset crash counter so pre-restart crashes don't count against the day limit
        const newEntry = this.agents.get(agentName);
        if (newEntry) {
          newEntry.process.resetCrashCount();
        }
        console.log(`[stale-watcher] ${agentName}: restart complete, crash count reset`);
      } catch (err) {
        console.error(`[stale-watcher] ${agentName}: restart failed: ${err}`);
      } finally {
        this.healthRestartInProgress.delete(agentName);
      }
    }, INTERVAL_MS);

    this.staleWatchers.set(agentName, timer);
    console.log(`[stale-watcher] Watcher started for ${agentName} (threshold: ${staleMinutes} min)`);
  }

  /**
   * Cancel and remove the stale-heartbeat watcher for `agentName`.
   * Called by `stopAgent()` so the watcher stops when the agent stops.
   */
  private stopStaleHeartbeatWatcher(agentName: string): void {
    const timer = this.staleWatchers.get(agentName);
    if (timer) {
      clearInterval(timer);
      this.staleWatchers.delete(agentName);
      console.log(`[stale-watcher] Watcher stopped for ${agentName}`);
    }
  }
  ...
  private async runHealthProbe(agentName: string): Promise<void> {
    ...
    try {
      ...
      // Clean up pending state so we don't false-positive on the ack check.
