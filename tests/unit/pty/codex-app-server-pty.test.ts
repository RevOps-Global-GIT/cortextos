...
describe('CodexAppServerPTY thread/tokenUsage/updated → context_status.json', () => {
  ...
  it('writes context_status.json with used_percentage from fresh (non-cached) per-turn input tokens', () => {
    const pty = new CodexAppServerPTY(mockEnv, {});
    (pty as unknown as { _threadId: string })._threadId = 'thread-9';
    // total is cumulative (would pin >100% and climb unbounded). last.inputTokens
    // includes prefix-cached tokens from prior sessions (5000); only the fresh
    // portion (70000 - 5000 = 65000) counts toward per-turn context fill.
    feedTokenUsage(pty, {
    ...
    expect(payload.used_percentage).toBeCloseTo(32.5, 5); // (70000 - 5000) / 200000 * 100
  ...
  it('flags exceeds_200k_tokens once per-turn last.inputTokens > 200k', () => {
    const pty = new CodexAppServerPTY(mockEnv, {});
    (pty as unknown as { _threadId: string })._threadId = 'thread-9';
    feedTokenUsage(pty, {
  ...
  it('clamps used_percentage to 100 when per-turn input exceeds cap', () => {
    const pty = new CodexAppServerPTY(mockEnv, {});
    (pty as unknown as { _threadId: string })._threadId = 'thread-9';
    feedTokenUsage(pty, {
  ...
  it('skips the write when neither last nor total usage block is present', () => {
    const pty = new CodexAppServerPTY(mockEnv, {});
    (pty as unknown as { _threadId: string })._threadId = 'thread-9';
    feedTokenUsage(pty, {
      modelContextWindow: 200000,
  ...
  it('falls back to cumulative total when the per-turn last block is absent', () => {
    const pty = new CodexAppServerPTY(mockEnv, {});
    (pty as unknown as { _threadId: string })._threadId = 'thread-9';
    feedTokenUsage(pty, {
