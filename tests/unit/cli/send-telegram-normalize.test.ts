...
async function parseSendTelegram(argv: string[]): Promise<void> {
  try {
    await busCommand.parseAsync(argv, { from: 'user' });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    if (!/process\.exit unexpectedly called with "?0"?/.test(message)) {
      throw err;
    }
  }
}
...
describe('PR-12: send-telegram normalizes literal \\n / \\t (codex agent fix)', () => {
  it('converts codex-style literal \\n into real newlines before sending', async () => {
    await parseSendTelegram(['send-telegram', '12345','hello\\n\\nworld']);
  ...
  it('converts codex-style literal \\t into real tabs before sending', async () => {
    await parseSendTelegram(['send-telegram', '12345','col1\\tcol2']);
  ...
  it('leaves real newlines untouched (claude-runtime no-op)', async () => {
    ...
    await parseSendTelegram(['send-telegram', '12345','line1\nline2']);
  ...
  it('preserves other escape sequences verbatim (e.g. \\r, \\xHH)', async () => {
    ...
    await parseSendTelegram(['send-telegram', '12345','has\\rcarriage']);
  ...
  it('handles mixed literal \\n and real \\n in the same message', async () => {
    await parseSendTelegram(['send-telegram', '12345','real\nthen\\nliteral']);
  ...
  it('normalizes the exact pattern observed in codex-research outbound log', async () => {
    ...
    await parseSendTelegram(['send-telegram', '12345',codexShape]);
