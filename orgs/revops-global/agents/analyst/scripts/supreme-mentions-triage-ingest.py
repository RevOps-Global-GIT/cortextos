...
def normalize_json_item(item: Dict[str, Any], scanned_at: str) -> Optional[Dict[str, Any]]:
    ...
    item_id = str(item.get("id") or "").strip() or stable_id(header, preview, link or message_ts)
    ...
    return {
        ...
        "slack_url": link,
        "is_question": bool(item.get("is_question")) or bool(question_markers.search(preview)),
