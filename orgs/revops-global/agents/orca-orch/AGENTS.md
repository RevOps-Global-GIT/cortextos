...
8. Check today's memory file for in-progress work — **read only the last 100 lines** to avoid loading a restart-loop-bloated file into context. Large daily memory files (>50KB) are a detection-shadow failure caused by rapid restart cycles appending session entries; loading them in full collapses context and causes the next restart sooner, compounding the loop.
   ```bash
   tail -100 "memory/$(date -u +%Y-%m-%d).md"
   ```
   If the file exceeds 50KB, archive the bulk to `memory/$(date -u +%Y-%m-%d)-archive-<reason>.md` before proceeding, then continue with the trimmed file.
