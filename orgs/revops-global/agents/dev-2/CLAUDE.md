...
**ZERO writes to grandamenium/cortextos. This means:**
- No pushes to grandamenium branches
- No PRs targeting grandamenium/cortextos (cross-fork PRs count as writes)
- No comments, reviews, or any GitHub API writes to grandamenium repos

**All PRs target RevOps-Global-GIT/cortextos only.** Never open a PR with `--repo grandamenium/cortextos` or with base pointing to grandamenium. If you find yourself about to do this, stop and re-target the fork.

**Branch discipline:** Never commit to `main`. Always `fix/<slug>`, `feat/<slug>`, `chore/<slug>` off main.

**Staging:** Always `git add <specific paths>` — never `git add -A` or `git add .`.
