...
- Run visual verification with agent-browser (Mac SSH Codex, `--ssh-host gregs-mac`, only for Mac-only state) for UI-adjacent work.
...
- Default to Greg's Mac for browser automation: use agent-browser first; Mac SSH (`gregs-mac`) only for Mac-only state.
