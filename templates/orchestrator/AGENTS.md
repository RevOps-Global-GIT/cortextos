...
These skills are invocation defaults — reach for them automatically when their trigger condition is met, without waiting for a prompt to mention them. As orchestrator, you set the standard for the fleet: invoking these explicitly drives the rest of the agents to follow.
